...
| M3: 醫師評估文件生成 | 可生成並下載「二位心臟外科專科醫師判定」Word 文件 | 未開始 |
| M4: 簽名文件上傳完成 | 可上傳已簽名的醫師評估文件 | 未開始 |
| M5: 事前審查文件生成 | 可生成完整「事前審查」Word 文件 (13 區塊) | 未開始 |
| M6: 核心功能完成 | 完整流程跑通 (輸入→生成→簽名→最終文件) | 未開始 |
| M7: MVP 完成 | 包含案例管理與 UI 優化 | 未開始 |
| M8: 正式上線 | 測試完成並部署到 Vercel | 未開始 |
...
| Phase 2: 外科醫師評估文件生成 | 未開始 | 0% | `/backend-dev` + `/ai-engineer` |
| Phase 3: 簽名文件上傳功能 | 未開始 | 0% | `/frontend-dev` |
| Phase 4: 事前審查文件生成 | 未開始 | 0% | `/backend-dev` + `/ai-engineer` |
...
### 立即執行（Phase 2）
1. **確認開發環境與 API Key**：
   - 確認用戶已取得 Claude API Key
   - 設定 `.env.local` 檔案

2. **開始 Phase 2 - 醫師評估文件生成**：
   - 建立 Claude API 封裝 (`lib/ai/claude.ts`)
   - 設計醫師評估摘要 Prompt（參考 PDF 範例）
   - 實作 docx.js Word 文件生成
   - 整合到 UI (步驟 7)

3. **準備測試資料**：
   - 使用已完成的 Phase 1 資料進行測試
   - 驗證生成的文件格式是否正確
...
### 未來規劃
- Week 3: 完成 Phase 4 (事前審查文件生成)
- Week 4: 完成 Phase 5-6 (案例管理與 UI 優化)
- Week 5: Phase 7 (測試與部署)
